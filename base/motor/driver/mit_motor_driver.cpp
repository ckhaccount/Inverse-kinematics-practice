...
// 查询电机ID(J23)
void MITMotorDriver::MotorGetID() {
  can_tx_header_.IDE = CAN_ID_STD;
  can_tx_header_.RTR = CAN_RTR_DATA;
  can_tx_header_.DLC = 4;
  can_tx_header_.StdId = 0x7FF;
...
// 查询电机通信模式，0x01：自动报文，0x02：问答(J123)
void MITMotorDriver::MotorGetCommMode() {
  can_tx_header_.IDE = CAN_ID_STD;
  can_tx_header_.RTR = CAN_RTR_DATA;
  can_tx_header_.DLC = 4;
  can_tx_header_.StdId = 0x7FF;

  can_tx_data_[0] = 0x00;    // 电机ID高8位
  can_tx_data_[1] = 0x04;    // 电机ID低8位
  can_tx_data_[2] = 0x00;    // 0x00(表示控制器发送的指令)
  can_tx_data_[3] = 0x81;    // 0x81

  HAL_CAN_AddTxMessage(hcan_, &can_tx_header_, can_tx_data_,
                       &can_tx_mail_box_);
}

// 设置电机通信模式，0x01：自动报文，0x02：问答(J123)
void MITMotorDriver::MotorCommModeSet() {
  can_tx_header_.IDE = CAN_ID_STD;
  can_tx_header_.RTR = CAN_RTR_DATA;
  can_tx_header_.DLC = 4;
  can_tx_header_.StdId = 0x7FF;

  can_tx_data_[0] = 0x00;    // 电机ID 高8位
  can_tx_data_[1] = 0x01;    // 电机ID低8位
  can_tx_data_[2] = 0x00;    // 0x00(表示控制器发送的指令)
  can_tx_data_[3] = 0x02;    // 0x01: 自动报文 0x02: 问答
...
// J123问答通信模式+力位混控控制模式
void MITMotorDriver::canTxMsg_OD_() {
  // can message header
  can_tx_header_.IDE = CAN_ID_STD;
  can_tx_header_.RTR = CAN_RTR_DATA;
  can_tx_header_.DLC = 8;
  ...
  uint8_t mode_ = 0x00;    // 电机模式，0: 力位混控 1: 伺服位置 2: 伺服速度 3: 电流、力矩和刹车

  // transmit control pack
  setTorque(motor_->intensity_float_);
  can_tx_data_[0] = (mode_ << 5) | (tx_data_.kp >> 7);
  can_tx_data_[1] = ((tx_data_.kp & 0x7F) << 1) | (tx_data_.kv >> 8);
  can_tx_data_[2] = tx_data_.kv & 0xFF;
  can_tx_data_[3] = tx_data_.p >> 8;
  can_tx_data_[4] = tx_data_.p & 0xFF;
  can_tx_data_[5] = tx_data_.v >> 4;
  can_tx_data_[6] = ((tx_data_.v & 0xF) << 4) | (tx_data_.t_ff >> 8);
  can_tx_data_[7] = tx_data_.t_ff & 0xFF;
  HAL_CAN_AddTxMessage(hcan_, &can_tx_header_, can_tx_data_,
                       &can_tx_mail_box_);
}
...
// J123问答模式
bool MITMotorDriver::canRxMsgCheck_OD_(CAN_HandleTypeDef* hcan,
                                      CAN_RxHeaderTypeDef rx_header) {
  return hcan == hcan_ && rx_header.StdId == master_id_;
}
...
// J123问答模式
void MITMotorDriver::canRxMsgCallback_OD_(CAN_HandleTypeDef* hcan,
                                         CAN_RxHeaderTypeDef rx_header,
                                         uint8_t rx_data[7]) {
  if (motor_->info_.type != Motor::OD || (rx_data[0] >> 5) != 0x01) {
    return;
  }
